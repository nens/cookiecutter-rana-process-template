...
from ..domain import RanaDatasetLizardRaster, RanaProcessParameters, ThreediApiKey
...
class LocalTestRanaContext(RanaContext[T], Generic[T]):
    ...
    def get_lizard_raster_dataset(self, id: Id) -> RanaDatasetLizardRaster:
        dataset = self._settings().datasets[str(id)]
    ...
    def schematisation_id(self, schematisation: ThreediSchematisation) -> Id:
        with open(self._rana_runtime.project_dir / schematisation.id) as schematisation_file:
            return schematisation_file.read().strip()
