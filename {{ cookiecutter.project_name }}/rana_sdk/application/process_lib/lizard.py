...
from clean_python import Id, Json
from clean_python.api_client import download_file
from osgeo import gdal

from ..rana_runtime import RanaRuntime
from rana_sdk.application.process_lib import get_bbox, get_epsg_code, get_projection, open_vector_file
from rana_sdk.domain import LizardRaster, LizardTask, ProcessUserError
...
def _check_raster_size(path: Path) -> None:
    ...
    if size > RASTER_MAX_SIZE_BYTES:
        from rana_sdk.domain import ProcessUserError
...
def download_raster(
    ...
    download_raster_by_bbox(
        lizard=runtime.lizard_raster_layer_gateway,
        raster=raster,
        bbox=bbox,
        epsg_code=epsg_code,
...
def get_task(lizard: LizardRasterLayerGateway, task_id: Id) -> LizardTask:
    """Fetches a task from Lizard and returns it as a LizardTask object.
    ...
    Returns:
        LizardTask: The fetched task.
    """
    task_json = lizard.get_task(task_id)
    return LizardTask(**task_json)
